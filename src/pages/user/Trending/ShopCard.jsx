  ...
  return (
    <div className='relative flex flex-col items-center gap-4 pb-5 cursor-pointer
      ...
      <div className='absolute top-4 right-1 bg-white rounded-full p-2'>
        <motion.img src={isInWishList ? FillHeart : Heart} whileTap={{ scale: 0.8 }} alt="wishlist" className='cursor-pointer hover:text-black'
          onClick={() => { isInWishList ? removeFromWishlist(id) : addToWishlist({ id, image, title, brand, price }) }} />
      </div>
    </div>
  )
}
