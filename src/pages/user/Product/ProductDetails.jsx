...
const ProductDetails = ({ details, selectedVariant, setSelectedVariant }) => {
    ...
    return (
        <section className='flex flex-col gap-4 '>
            ...
            <h3 className='text-[#546D7D] text-base font-normal'>{selectedVariant?.brandName || details?.brandName}</h3>
            <h3 className='text-[#A5B2BA] text-base font-normal'>{selectedVariant?.categoryName || details?.categoryName}</h3>
                        ...
                        <div className='flex gap-5'>
                            {details.variants.map((variant, index) => (
                                <div key={index} onClick={() => setSelectedVariant(variant)}>
                                    <div
                                        className={`h-12 w-12 cursor-pointer border ${selectedVariant?.variantId === variant.variantId
                                            ? 'border-[#ca8904] border-2'
                                            : 'border-gray-200'
                                            }`}
                                        style={{ backgroundColor: variant.hexCode }}
                                    ></div>
                                    <h4 className='text-[#605D5D] text-xs mt-1'>{variant.colorName}</h4>
                                </div>
                            ))}
                        </div>
                    </div>
            ...
            <div className='font-gilroy py-2'>
                <div className='flex gap-2 items-center'>
                    ...
                    <h1 className='text-base text-[#757C81]'>Standard delivery between <span className='font-semibold text-[#010F17]'>With In 5 Days</span></h1>
                </div>
                <div className='flex gap-2 items-center'>
                    <div>
                        <img src={DoubleTick} alt="doubletick" />
