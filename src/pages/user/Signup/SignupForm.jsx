  ...
  const handleSubmit = (values) => {
    ...
    mutate(payload, {
      onSuccess: (data) => {
        ...
        // The login page will handle the final redirect after successful login
        navigate("/login");
      },
      onError: (error) => {
      ...
      <Form>
        ...
        <div className=" space-y-4">
          <div className="grid grid-cols-1">
            <InputFieldAuth name="fullName" placeholder="Name" />
          </div>
          <div className="grid grid-cols-1">
            <InputFieldAuth name="email" placeholder="Email ID" type="email" />
          </div>
          <div className="grid grid-cols-1">
            <InputFieldAuth name="phoneNumber" placeholder="Contact Number" />
          </div>
          <div className="grid grid-cols-1">
            <InputFieldAuth name="passwordHash" placeholder="Password" />
          </div>
          <div className="grid grid-cols-1">
            <InputFieldAuth
              name="repeatPassword"
          ...
          <div className="bg-[#005C53] cursor-pointer text-white flex mx-auto w-fit mt-10 rounded-full transition-all duration-300 ease-in-out hover:bg-[#004A42] hover:shadow-lg hover:scale-105">
            <button
              type="submit"
              className="uppercase py-3 px-10 tracking-wider "
            >
              Sign up
            </button>
          </div>
