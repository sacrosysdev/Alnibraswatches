...
const ManageAddress = () => {
  ...
  const { mutate: updateDefault, isPending: updatingDefault } = useUpdateDefaultAddress();
  ...
  const selectedDefaultId = defaultAddressId ||
  ...
  return (
    <div>
      <div className='text-black font-bold text-xl mb-8 pt-5'>Manage Address</div>
      {addresses.map((item) => {
        let parsedDetails = {}
        try {
          ...
          <div key={item.AddressId} className='mb-4 flex flex-col md:flex-row'>
            ...
            <div className='flex items-center mt-2 md:mt-0 md:ml-4'>
              ...
              <label className='ml-2 text-sm font-medium text-gray-700'>
                {selectedDefaultId === item.AddressId ? 'Default' : 'Change as Default'}
              </label>
            </div>
          </div>
        )
      ...
      {showModal && (
        <AddressModal
          initialValues={isEditing && currentAddress ? collectEditValues(currentAddress.AddressId) : initialValues}
          onSubmit={onSubmit}
          handleCloseModal={handleCloseModal}
          isEditing={isEditing}
          addressId={isEditing && currentAddress ? currentAddress.AddressId : null}
        />
      )}
    </div>
  )
