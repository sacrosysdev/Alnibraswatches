...
const Sidebar = ({ headerHeight = 64 }) => {
    ...
    useEffect(() => {
        const handleClickOutside = (e) => {
            if (isOpen && isMobile && !e.target.closest('.sidebar-container') && !e.target.closest('.sidebar-toggle')) {
                setIsOpen(false);
            }
        };
    ...
    return (
        <>
            ...
            {isMobile && (
                ...
                >
                    {isOpen ? <X size={24} /> : <RiMenuFoldFill size={30} color='#005C53' />}
                </button>
            )}

            {/* Sidebar - always visible on desktop, conditional on mobile */}
            <div
                className={`sidebar-container transition-all duration-300 ease-in-out ${isMobile
                        ? `fixed top-0 left-0 h-screen z-50 bg-white shadow-xl transform ${isOpen ? 'translate-x-0' : '-translate-x-full'
                        }`
                        : 'relative'
                    }`}
            ...
            >
                <div className='w-72 font-source p-4'>
                    <div className='flex flex-col items-center gap-2'>
                        <div className=''>
                            <img src={Avatar} alt="avatar" className='h-24 w-24 rounded-full bg-gray-300 shadow-xl' />
                        </div>
